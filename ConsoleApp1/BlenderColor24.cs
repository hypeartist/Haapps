...
{
	public readonly unsafe ref struct BlenderColor24
	{
		private static readonly Bgr Order = default;

		public void BlendPixel(byte* p, int r, int g, int b, int a, int c)
		{
			var pdr = p[Order.R];
			var pdg = p[Order.G];
			var pdb = p[Order.B];
			...
			Debug.WriteLine($"src=({r},{g},{b},{a}), dst=({pdr},{pdg},{pdb}), cover={c}");
			Debug.WriteLine($"\tsr-dr={r - pdr}, (sr-dr)*sa={(r - pdr) * a}");
			Debug.WriteLine($"\tsg-dg={g - pdg}, (sg-dg)*sa={(g - pdg) * a}");
			Debug.WriteLine($"\tsb-db={b - pdb}, (sb-db)*sa={(b - pdb) * a}");
			Debug.WriteLine($"res=({p[Order.R]},{p[Order.G]},{p[Order.B]})");
			Debug.WriteLine("--------------------------------");
		}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void BlendPixel8(byte* pDst, Color8 pSrc, byte cover)
		{

...
{
	public unsafe struct RefPodList<T> : IDisposable
		where T:unmanaged
	{
		private readonly int _blockSize;
		private T* _data;

		public RefPodList(int blockSize, bool preAllocate = true)
		{
			_blockSize = blockSize;
			_data = DataPtr = default;
			Count = Size = 0;
		...
		public void Add(ref T element)
		{
			if (Count + 1 > Size)
			{
				var tmp = (T*) PodHeap.AllocateRaw<T>(Size = Count + _blockSize);

...
namespace Haapps.Gfx.Agg
{
	public static unsafe class Common
	{
		...
		public const int CoverNone = 0;
		public const int CoverFull = CoverMask;

		public const int PolySubpixelShift = 8;
		public const int PolySubpixelShiftMul2 = PolySubpixelShift << 1;
		public const int PolySubpixelScale = 1 << PolySubpixelShift;
		public const int PolySubpixelMask = PolySubpixelScale - 1;

		public const int AAShift = 8;
		public const int AAScale = 0x100;
		public const int AAScale2 = 0x200;
		public const int AAMask = 0xff;
		public const int AAMask2 = 0x1ff;

		public const int SubpixelShift = LineAA.LineSubpixelShift;
		public const int SubpixelScale = 1 << SubpixelShift;
		public const int SubpixelScaleMul2 = SubpixelScale << 1;

		public const int GradientSubpixelShift = 4;
		public const int GradientSubpixelScale = 1 << GradientSubpixelShift;
		public const int GradientSubpixelMask = GradientSubpixelScale - 1;
		public const int DownscaleShift = 8 - GradientSubpixelShift;
		...
		{
			var cp1 = CrossProduct(x1, y1, x2, y2, x, y) < 0.0;
			var cp2 = CrossProduct(x2, y2, x3, y3, x, y) < 0.0;
			var cp3 = CrossProduct(x3, y3, x1, y1, x, y) < 0.0;
		...
		public static void DilateTriangle(double x1, double y1, double x2, double y2, double x3, double y3, double* x, double* y, double d)
		{
			var dx1 = 0.0;
			var dy1 = 0.0;
			var dx2 = 0.0;
			...
			var dx3 = 0.0;
			var dy3 = 0.0;
			var loc = CrossProduct(x1, y1, x2, y2, x3, y3);
			if (Math.Abs(loc) > IntersectionEpsilon)
			{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining|MethodImplOptions.AggressiveOptimization)]
		public static void DilateTriangle(double x1, double y1, double x2, double y2, double x3, double y3, PathItem* pathItems, double d)
		{
			var dx1 = 0.0;
			var dy1 = 0.0;
			var dx2 = 0.0;
			...
			pathItems[0].X = x1 + dx1;
			pathItems[0].Y = y1 + dy1;
			pathItems[1].X = x2 + dx1;
			pathItems[1].Y = y2 + dy1;
			pathItems[2].X = x2 + dx2;
			pathItems[2].Y = y2 + dy2;
			pathItems[3].X = x3 + dx2;
			pathItems[3].Y = y3 + dy2;
			pathItems[4].X = x3 + dx3;
			pathItems[4].Y = y3 + dy3;
			pathItems[5].X = x1 + dx3;
			pathItems[5].Y = y1 + dy3;
		}
...
// #if DEBUG
// 		public static void DebugPrint<TOrder>(byte* p, int x, int y, int length)
// 			where TOrder : struct, IOrderRGB
// 		{
// 			TOrder order = default;
// 			var sb = new StringBuilder(length);
// 			for (var i = 0; i < length; i++)
// 			{
// 				sb.Append(p[order.R].ToString("X2"));
// 				sb.Append(p[order.G].ToString("X2"));
// 				sb.Append(p[order.B].ToString("X2"));
// 				p+=3;
// 			}
// 			System.Diagnostics.Debug.WriteLine($"x:{x}, y:{y}, len:{length}, bytes: {sb}");
// 		}
// #endif
	}
}
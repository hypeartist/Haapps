...
{
	public struct RasterizerSlClip32<TRasConv> : IRasterizerClipper32<TRasConv>
		where TRasConv : unmanaged, IRasterizerConverter32
	{
		private static TRasConv _conv = default;

		private Rectangle32 _clipRect;
		...
		{
			_clipRect = new Rectangle32 {X1 = x1, Y1 = y1, X2 = x2, Y2 = y2};
			_clipRect.Normalize();
			_clipping = true;
		}

		private void LineClipY(ref RasterizerCellsAA<Cell> ras, int x1, int y1, int x2, int y2, int f1, int f2)
		{
			f1 &= 10;
			f2 &= 10;
			if ((f1 | f2) == 0)

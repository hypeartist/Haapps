...
namespace Haapps.Gfx.Agg
{
		...
		where TBlenderColor : unmanaged, IBlenderColor24<TOrderColor>
	{
		...
		public PixfmtAlphaBlend24(ref TRenderingBuffer buffer)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining|MethodImplOptions.AggressiveOptimization)]
		public readonly byte* GetPixPtr(int x, int y) => _buffer->GetRowPtr(y) + (uint) (x * BytesPerPixel);
		...
		{
			var p = _buffer->GetRowPtr(y) + (uint)(x + x + x);
		...
		{
			var p = _buffer->GetRowPtr(x, y, 1) + (uint)(x + x + x);
		...
		{
			var p = _buffer->GetRowPtr(x, y, length) + (uint)(x + x + x);
		...
		public readonly void BlendHLine(int x, int y, int length, Color8 color, byte cover)
		{
			...
			// while (length >= 4)
			// {
			// 	_blender.BlendPixel4(p, color, cover);
			// 	p += 12;
			// 	length -= 4;
			// }
			//
		...
		public readonly void BlendSolidHSpan(int x, int y, int length, Color8 color, byte* covers)
		{
			...
			// while (length >= 4)
			// {
			// 	_blender.BlendPixel4(p, color, covers);
			// 	p += 12;
			// 	length -= 4;
			// 	covers += 4;
			// }

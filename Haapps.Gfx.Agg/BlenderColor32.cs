...
{
	public readonly unsafe struct BlenderColor32<TOrder> : IBlenderColor32<TOrder>
		where TOrder : unmanaged, IOrderColor32
	{
		private static readonly TOrder Order = default;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
		public void BlendPixel(byte* p, int r, int g, int b, int a, int cover) => BlendPixel(p, r, g, b, a);
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
		public void BlendPixel4(ref byte* pDst, ref int length, Color8 color, ref byte* covers)
		{
			var vSrc = Ssse3.Shuffle(Vector128.Create(*(int*) &color).AsByte(), mskSrcShfl);

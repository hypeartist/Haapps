...
{
	public sealed unsafe class GSVText : VertexSourceAbstract
	{
		private static readonly byte[] GsvDefaultFont =
		{
		...
		private byte* _text;
		private PodStringAnsi _textBuffer;
		private bool _textBufferIsInternal;
		private byte* _curChr;
		private readonly PodArray<byte> _font = new PodArray<byte>(GsvDefaultFont.Length);
		private Status _status;
		private readonly bool _bigEndian = *(char*)Unsafe.AsPointer(ref T) == 0;
		private byte* _indices;
		private byte* _glyphs;
		private byte* _bglyph;
		private byte* _eglyph;
		private double _w;
		private double _h;
		...
		{
			Flip = false;
			Unsafe.CopyBlock(_font.DataPtr, Unsafe.AsPointer(ref GsvDefaultFont[0]), (uint) GsvDefaultFont.Length);
		...
		{
			_font.Dispose();
			if(!_textBufferIsInternal) return;
			PodHeap.Free(_textBuffer.Chars);
			_textBuffer = default;
		}

		public void Size(double height, double width = 0)
		{
			_width = width;
			_height = height;
		}
		...
		public double TextWidth
		{
			get
			{
				Bounds(0, out double x1, out _, out var x2, out _);
				return x2 - x1;
			}
		}
		...
		private short Value(byte* p)
		{
			short v = 0;
			var pv = (byte*)Unsafe.AsPointer(ref v);
			if (_bigEndian)
			{
				*pv = p[1];
				*(pv + 1) = p[0];
		...
		public void SetContent(string value)
		{
			...
			{
				_textBufferIsInternal = false;
				return;
			}
			_textBufferIsInternal = true;
			_textBuffer = PodStringAnsi.FromMangedString(value);
			_text = _textBuffer.Chars;
		}

		public void SetContent(PodStringAnsi value)
		{
			_textBufferIsInternal = false;
			_textBuffer = value;
			_text = _textBuffer.Chars;
		...
		{
			const int offset = 257*2;
			_status = Status.Initial;
			_indices = _font;
			double baseHeight = Value(_indices + 4);
			_indices += Value(_indices);
			_glyphs = _indices + offset;
			_h = _height/baseHeight;
			_w = (Math.Abs(_width) < double.Epsilon) ? _h : _width/baseHeight;
			if (Flip) _h = -_h;
			_curChr = _text;
		}
		...
		public override PathCommand Vertex(ref double x, ref double y)
		{
			...
			while (!quit)
			{
				...
				{
					case Status.Initial:
						_status = Status.NextChar;
						break;

					case Status.NextChar:
						if (_curChr == null || *_curChr == 0)
						{
							quit = true;
							break;
						}
						var idx = *_curChr++ & 0xFF;
						if (idx == '\n')
						{
							_x = _startX;
							_y -= Flip ? -_height - LineSpace : _height + LineSpace;
							break;
						}
						idx <<= 1;
						_bglyph = _glyphs + Value(_indices + idx);
						_eglyph = _glyphs + Value(_indices + idx + 2);
						_status = Status.StartGlyph;
						break;
					case Status.StartGlyph:
						x = _x;
						y = _y;
						_status = Status.Glyph;
					...
					case Status.Glyph:
						...
						var dx = (sbyte) *_bglyph++;
						sbyte yc;
						var yf = (yc = (sbyte) *_bglyph++) & 0x80;
						yc <<= 1;
						yc >>= 1;
						int dy = yc;
						_x += dx*_w;
						...
						x = _x;
						y = _y;
						return yf != 0 ? PathCommand.MoveTo : PathCommand.LineTo;
				}
			}
